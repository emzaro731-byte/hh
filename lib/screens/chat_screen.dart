...
class _ChatScreenState extends State<ChatScreen> {
  ...
  static const _tools = <Map<String, dynamic>>[
    ...
    },
    {
      ...
      'hint': 'Generate videos',
    },
  ];

  static const _models = <Map<String, String>>[
    {
      'name': 'GPT-OSS 120B',
    ...
    },
    {
      'name': 'Compound',
      'id': 'groq/compound',
      'desc': 'AI system with tools',
    },
  ];

  String get _modelId =>
      _models.firstWhere((m) => m['name'] == _model)['id']!;
  ...
  Future<void> _send() async {
    ...
    if (_tool == 'Image') {
      await _media(false, text);
      return;
    }
    if (_tool == 'Video') {
      await _media(true, text);
      return;
    }
    if (_tool == 'Music') {
      await _music(text);
        ...
        messages: const [
          {
            'role': 'system',
            'content':
                'You are VEYLORA Music Studio. Create an original song blueprint with title, genre, BPM, structure, lyrics and production direction. Never copy existing lyrics.',
          },
        ],
      ).then((base) => base);
  ...
  Future<void> _media(bool video, String prompt) async {
    ...
    try {
      final task = video
          ? await _ai.generateVideo(prompt)
          : await _ai.generateImage(prompt);
  ...
  Future<void> _save() async {
    await widget.prefs.setString('messages', jsonEncode(_messages));
  }
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          const Text(
            'Chat, create images, make music, produce videos — all in one place.',
            style: TextStyle(
          ...
          Wrap(
            ...
            children: [
              _idea(
                ...
                'Give me 10 powerful creative ideas.',
              ),
              _idea(
                'Create a logo',
                Icons.image_outlined,
                'Create a premium logo concept for my brand.',
              ),
              _idea(
                'Make an Afrobeat song',
                Icons.music_note_outlined,
                'Make a fresh original Afrobeat song concept.',
              ),
              _idea(
                'Generate a video',
                Icons.movie_outlined,
                'Generate a cinematic futuristic video.',
              ),
            ],
          ),
        ],
  ...
  Widget _idea(String label, IconData icon, String text) {
    return ActionChip(
      avatar: Icon(icon, size: 17),
      label: Text(label),
      onPressed: () => _quick(text),
    );
  }
        ...
        children: [
          if (!user)
            const Padding(
              ...
              child: CircleAvatar(
                radius: 15,
                backgroundColor: Color(0xFF222947),
                child: Icon(
                  Icons.auto_awesome,
                  size: 15,
  ...
  Widget _composer() {
    return SafeArea(
      child: Padding(
        ...
        child: Container(
          padding: const EdgeInsets.fromLTRB(14, 9, 9, 8),
          decoration: BoxDecoration(
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  _selector(_tool, Icons.tune_rounded, _showTools),
                  const SizedBox(width: 5),
                  _selector(_mode, Icons.bolt, _showModes),
                  const SizedBox(width: 5),
                  _selector(_model, Icons.auto_awesome, _showModels),
                  const Spacer(),
                  IconButton(
                    onPressed: () {},
  ...
  Widget _selector(String text, IconData icon, VoidCallback onTap) {
    final isModel = _models.any((model) => model['name'] == text);
    return GestureDetector(
      onTap: onTap,
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: isModel
                ? const Color(0xFF22E878)
                : const Color(0xFF596277),
          ),
        ),
        child: Row(
        ...
        child: Column(
          ...
          children: _tools.map((tool) {
            ...
            return ListTile(
              ...
              title: Text(name),
              subtitle: Text(tool['hint'] as String),
              trailing: _tool == name
                  ? const Icon(Icons.check, color: Color(0xFF22E878))
                  : null,
              onTap: () {
                setState(() => _tool = name);
  ...
  void _showModes() {
    const modes = ['Fast', 'Balanced', 'Creative', 'Deep', 'Auto'];
    showModalBottomSheet<void>(
      context: context,
      backgroundColor: const Color(0xFF121722),
      builder: (context) => SafeArea(
              ...
              ..._models.map(
                (model) => ListTile(
                  leading: const CircleAvatar(
                    backgroundColor: Color(0xFF1D2940),
                    child: Icon(
                      Icons.auto_awesome,
  ...
  Widget _profileMenu() {
    final email =
        Supabase.instance.client.auth.currentUser?.email ?? 'VEYLORA user';
    return Positioned(
      left: 6,
      top: 65,
      child: Material(
        ...
        child: Container(
          ...
          child: Column(
            children: [
              ListTile(
                leading: const CircleAvatar(
                  backgroundColor: Color(0xFF47338D),
              ...
              ListTile(
                ...
                onTap: () => _message('Profile coming soon'),
              ),
              ListTile(
                leading: const Icon(Icons.settings_outlined),
                title: const Text('App Settings'),
                onTap: () => _message('Settings coming soon'),
              ),
              ListTile(
