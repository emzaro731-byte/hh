...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void initState() {
    ...
    if (saved != null) { try { _messages = List<Map<String, String>>.from((jsonDecode(saved) as List).map((e) => Map<String, String>.from(e))); } catch (_) {} }
  }
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: Column(children: [Expanded(child: _messages.isEmpty ? _welcome() : ListView.builder(controller: _scroll, padding: const EdgeInsets.fromLTRB(16, 12, 16, 18), itemCount: _messages.length, itemBuilder: (c, i) => _bubble(_messages[i]))), if (_busy) const Padding(padding: EdgeInsets.only(bottom: 8), child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [SizedBox(width: 14, height: 14, child: CircularProgressIndicator(strokeWidth: 2)), SizedBox(width: 8), Text('VEYLORA is working…', style: TextStyle(color: Colors.white60))])), _composer()]),
  );

  Widget _welcome() => SingleChildScrollView(child: Padding(padding: const EdgeInsets.fromLTRB(22, 45, 22, 20), child: Column(children: [Container(width: 82, height: 82, decoration: BoxDecoration(shape: BoxShape.circle, gradient: const LinearGradient(colors: [Color(0xFF9C83FF), Color(0xFF5D7CFF)]), boxShadow: [BoxShadow(color: const Color(0xFF8065FF).withOpacity(.3), blurRadius: 30)]), child: const Icon(Icons.auto_awesome, size: 42)), const SizedBox(height: 20), const Text('Meet VEYLORA', style: TextStyle(fontSize: 32, fontWeight: FontWeight.w900)), const SizedBox(height: 8), const Text('One creative AI workspace for ideas, conversations and generation.', textAlign: TextAlign.center, style: TextStyle(color: Colors.white60, fontSize: 15, height: 1.5)), const SizedBox(height: 28), Wrap(spacing: 9, runSpacing: 9, alignment: WrapAlignment.center, children: [_PromptChip(label: 'Write a song', icon: Icons.music_note, text: 'Write an original Afro-fusion song about ambition.', onTap: _quick), _PromptChip(label: 'Create an image', icon: Icons.image_outlined, text: 'Create a cinematic futuristic Nigerian city at night.', onTap: _quick), _PromptChip(label: 'Create a video', icon: Icons.movie_outlined, text: 'Create a cinematic 5 second futuristic music video.', onTap: _quick), _PromptChip(label: 'Build an idea', icon: Icons.lightbulb_outline, text: 'Give me a powerful startup idea I can build.', onTap: _quick)])])));
...
class _PromptChip extends StatelessWidget {
  final String label, text; final IconData icon; final ValueChanged<String> onTap;
  const _PromptChip({required this.label, required this.icon, required this.text, required this.onTap});
  @override Widget build(BuildContext context) => ActionChip(avatar: Icon(icon, size: 17), label: Text(label), onPressed: () => onTap(text));
}