...
class _ChatScreenState extends State<ChatScreen>{
  ...
  const _ai=AiService(); List<Map<String,String>> _messages=[]; bool _busy=false;
  ...
  Future<void> _media(bool video) async{final prompt=_controller.text.trim();if(prompt.isEmpty||_busy)return;_controller.clear();setState(()=>_busy=true);try{final task=video?await _ai.generateVideo(prompt):await _ai.generateImage(prompt);setState(()=>_messages.add({'role':'assistant','content':'✨ VEYLORA ${video?'Video':'Image'} generation started.\nTask ID: $task','id':_uuid.v4()}));await _save();}catch(e){_error(e);}finally{if(mounted)setState(()=>_busy=false);}}
