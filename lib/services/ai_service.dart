...
class AiService {
  ...
  Future<dynamic> _post(String fn, Map<String, dynamic> body, {Duration timeout = const Duration(seconds: 90)}) async {
    ...
    if (r.statusCode < 200 || r.statusCode >= 300) {
      throw Exception('VEYLORA $fn error ${r.statusCode}: ${r.body}');
    }
    return jsonDecode(r.body);
  }

  Future<String> chat({required List<Map<String, String>> messages, String model = 'groq/compound'}) async {
  ...
  Future<String> generateMusic(String prompt, {String duration = '30', bool customMode = false, bool instrumental = false, String style = 'Afrobeats, modern, polished, cinematic', String title = 'VEYLORA AI', String model = 'V6'}) async {
    final data = await _post('generate-music', {
      'prompt': prompt,
    ...
    return (data['data']?['task_id'] ?? data['data']?['taskId'] ?? data['taskId'] ?? data['url'] ?? '').toString();
  }

  Future<String> generateVideo(String prompt, {String ratio = '16:9', int duration = 5, String resolution = '1080p', String? imageUrl}) async {
    final data = await _post('generate-video', {
      'prompt': prompt,
      'ratio': ratio,
      'duration': duration,
    ...
    return (data['data']?['task_id'] ?? data['data']?['taskId'] ?? data['taskId'] ?? '').toString();
  }
}
