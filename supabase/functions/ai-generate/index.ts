...
const MODELS = {
  image: [
    { id: 'z-image', name: 'Z-Image Turbo', speed: 'Fast', tier: 'free', provider: 'KIE' },
...
const PREMIUM_PLANS = new Set(['go', 'plus', 'ultra']);

function isPremiumPlan(plan: string) {
  return PREMIUM_PLANS.has(plan);
}

function findModel(type: string, id: string | undefined) {
  return ((MODELS as any)[type] || []).find((m: any) => m.id === id);
}
...
serve(async req => {
  ...
  try {
    ...
    if (service && auth) {
      ...
      userId = user?.id || null;
      if (userId) {
        const { data: profile } = await service.from('profiles').select('ai_plan').eq('id', userId).maybeSingle();
        plan = profile?.ai_plan || 'free';
      }
    }
    const premium = isPremiumPlan(plan);

    if (action === 'models') {
      const chat = await groqModels();
      return json({
        plan,
        premium,
        models: MODELS,
        ...
        availableModels: {
          image: premium ? MODELS.image : MODELS.image.filter((m: any) => m.tier === 'free'),
          video: premium ? MODELS.video : MODELS.video.filter((m: any) => m.tier === 'free'),
          music: premium ? MODELS.music : MODELS.music.filter((m: any) => m.tier === 'free'),
        },
        chat,
        presets: [
          { id: 'auto', name: premium ? 'Premium Auto' : 'Free Auto', speed: premium ? 'Premium quality' : 'Free supported model' },
          { id: 'fast', name: 'Fast', speed: premium ? 'Fast model' : 'Free supported model' },
    ...
    // The server decides the user's tier. Client-supplied freeMode/plan values
    // are deliberately ignored so Free users cannot unlock Premium KIE models.
    if (type === 'image' || type === 'video' || type === 'music') {
      let model = FREE_MODELS[type];
      if (premium) {
        const requested = options.model as string | undefined;
        const selected = findModel(type, requested);
      ...
      if (type === 'image') {
        ...
        return json({ taskId: id, status: 'pending', model, tier: premium ? 'premium' : 'free', plan, raw: d });
      }

      if (type === 'video') {
        const input: any = {
          prompt,
          resolution: options.quality === '1080p' && premium ? '1080p' : '720p',
    ...
    const requestedChat = options.model || 'openai/gpt-oss-20b';
    const model = premium
      ? requestedChat
      : 'openai/gpt-oss-20b';
    ...
    return json({ message: d.choices?.[0]?.message?.content || '', status: 'success', model: d.model || model, plan, tier: premium ? 'premium' : 'free', raw: d });
  } catch (e) {
