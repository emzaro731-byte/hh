...
async function googleAccessToken(){
  ...
  if(!tokenRes.ok)throw new Error(tokenJson.error_description||'Could not obtain Firebase access token');
...
serve(async req=>{
  ...
  try{
    ...
    const {data:call,error:callError}=await service.from('calls').select('id,caller_id,callee_id,type,status').eq('id',callId).single();
    if(callError||!call)return json({error:'Call not found'},404);
    if(call.caller_id!==user.id)return json({error:'Only the caller can send the incoming call push'},403);
    ...
    const deviceTokens=(tokens||[]).map((x:any)=>x.token).filter(Boolean);
    if(!deviceTokens.length)return json({sent:0,message:'Callee has no registered push token'});
    ...
    for(const token of deviceTokens){
      const response=await fetch(`https://fcm.googleapis.com/v1/projects/${projectId}/messages:send`,{
        ...
        body:JSON.stringify({message:{token,notification:{title:`Incoming ${call.type==='video'?'video':'voice'} call`,body:`${callerName} is calling you`,sound:'default'},data:{type:'incoming_call',callId:call.id,callerId:call.caller_id,callerName,callType:call.type},android:{priority:'high',notification:{channel_id:'gg-incoming-calls',sound:'default',notification_priority:'PRIORITY_MAX',default_sound:true,default_vibrate_timings:true,sticky:true,tag:`call-${call.id}`}},apns:{headers:{'apns-priority':'10','apns-push-type':'alert'},payload:{aps:{sound:'default',content_available:true}}}}}},
      });
      const body=await response.json().catch(()=>({}));
      results.push({token:token.slice(0,8),ok:response.ok,error:response.ok?undefined:body});
      if(response.status===404||response.status===410)await service.from('device_tokens').delete().eq('token',token);
    }
    return json({sent:results.filter(x=>x.ok).length,results});
  }catch(e){return json({error:e instanceof Error?e.message:'Could not send call push'},500)}
});
