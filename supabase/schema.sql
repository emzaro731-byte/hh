...
create table if not exists public.conversation_invites (id uuid primary key default gen_random_uuid(), conversation_id uuid not null references public.conversations(id) on delete cascade, created_by uuid not null references auth.users(id) on delete cascade, token text not null unique, created_at timestamptz not null default now());
create index if not exists conversation_invites_conversation_idx on public.conversation_invites(conversation_id);
create index if not exists conversation_invites_token_idx on public.conversation_invites(token);

alter table public.profiles enable row level security; alter table public.conversations enable row level security; alter table public.conversation_members enable row level security; alter table public.messages enable row level security; alter table public.conversation_invites enable row level security;
...
create policy "members can create invites" on public.conversation_invites for insert to authenticated with check (created_by=auth.uid() and exists(select 1 from public.conversation_members m where m.conversation_id=conversation_id and m.user_id=auth.uid()));
create policy "authenticated users can read invites" on public.conversation_invites for select to authenticated using (true);
