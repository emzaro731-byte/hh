...
class _AIPageState extends State<AIPage> {
  ...
  List<_Model> get models {
    if (mode == 'chat') return chatModels;
    if (mode == 'image') return imageModels;
    if (mode == 'video') return videoModels;
    return musicModels;
  }

  _Model get selected => models.firstWhere(
        (m) => m.id == model,
        orElse: () => models.first,
      );
  ...
  void chooseMode(String value) {
    setState(() {
      ...
      result = null;
      status = '';
      model = modelsFor(value).first.id;
    });
  }

  List<_Model> modelsFor(String value) {
    if (value == 'chat') return chatModels;
    if (value == 'image') return imageModels;
    if (value == 'video') return videoModels;
    return musicModels;
  }
  ...
  Future<void> run() async {
    ...
    try {
      var done = Map<String, dynamic>.from(
  ...
  IconData icon(String value) {
    if (value == 'image') return Icons.image_outlined;
    if (value == 'video') return Icons.movie_creation_outlined;
    if (value == 'music') return Icons.music_note_rounded;
  ...
  String extFor(String type, String url) {
    final c = url.split('?').first.toLowerCase();
    final d = c.lastIndexOf('.');
    if (d > 0 && d < c.length - 1 && c.length - d <= 6) {
      return c.substring(d + 1);
    }
  ...
  Future<void> download(String url, String type) async {
    ...
    try {
      ...
      try {
        ...
        final name = 'gg_${type}_${DateTime.now().millisecondsSinceEpoch}.$ext';
        final saved = await FilePicker.platform.saveFile(
          dialogTitle: 'Save ${title(type)}',
          fileName: name,
          bytes: Uint8List.fromList(b.takeBytes()),
        );
        if (mounted) {
  ...
  Widget mediaActions(List<String> media) {
    return Wrap(
      spacing: 8,
      runSpacing: 8,
      children: [
        for (final u in media)
        ...
        for (final u in media)
          FilledButton.icon(
            onPressed: downloading ? null : () => download(u, mode),
            icon: Icon(
              downloading ? Icons.hourglass_top : Icons.download_rounded,
            ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            onPressed: loading || downloading ? null : loadHistory,
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'One studio. Multiple models. Switch anytime.',
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (_, i) {
                        ...
                        return GestureDetector(
                          onTap: loading ? null : () => chooseMode(v),
                  ...
                  Card(
                    ...
                    child: InkWell(
                      ...
                      onTap: loading ? null : chooseModel,
                      child: Padding(
                        padding: const EdgeInsets.all(14),
                        child: Row(
                          children: [
                            CircleAvatar(child: Icon(icon(mode))),
                  ...
                  Row(
                    children: [
                      for (final p in ['auto', 'fast', 'expert'])
                        Expanded(
                          child: Padding(
                    ...
                    for (final h in history)
                      Card(
                        ...
                        child: ExpansionTile(
                          ...
                          subtitle: Text(
                            '${title('${h['type'] ?? 'chat'}')} • ${h['model'] ?? 'GG AI'}',
                          ),
                          children: [
                            ...
                            if ((h['result_text'] ?? '').toString().isNotEmpty)
                              Padding(
                                ...
                                child: Align(
                                  ...
                                  child: Text(h['result_text'].toString()),
                                ),
                              ),
                          ],
                        ),
          ...
          children: [
            ...
            Flexible(
              child: ListView.builder(
                ...
                itemBuilder: (_, i) {
                  final m = list[i];
                  final sel = m.id == widget.selected;
                  return ListTile(
                    leading: CircleAvatar(
                      child: Icon(
                        widget.mode == 'image'
