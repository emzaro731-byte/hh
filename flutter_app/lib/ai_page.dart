...
class _AIPageState extends State<AIPage> {
  ...
  String mode='chat',model='',status=''; bool loading=false,downloading=false;
  ...
  Future<void> download(String url,String type,{String? name}) async {
    ...
    try{
      final client=HttpClient();final request=await client.getUrl(Uri.parse(url));final response=await request.close();
  ...
  Widget mediaActions(List<String> media,String type,{String? name})=>Wrap(spacing:8,runSpacing:8,children:[for(final u in media)FilledButton.tonalIcon(onPressed:downloading?null:()=>open(u),icon:Icon(type=='video'?Icons.play_circle:type=='music'?Icons.headphones:Icons.open_in_new),label:Text(type=='video'?'Watch':type=='music'?'Listen':'Open')),for(final u in media)FilledButton.icon(onPressed:downloading?null:()=>download(u,type,name:name),icon:Icon(downloading?Icons.hourglass_top:Icons.download_rounded),label:Text(downloading?'Saving…':'Download'))]);
  @override Widget build(BuildContext context){final cs=Theme.of(context).colorScheme,media=urls(result);return Scaffold(
    appBar:AppBar(title:const Text('GG AI Studio',style:TextStyle(fontWeight:FontWeight.w900)),actions:[IconButton(onPressed:loading||downloading?null:loadHistory,icon:const Icon(Icons.history))]),
    body:CustomScrollView(slivers:[SliverToBoxAdapter(child:Padding(padding:const EdgeInsets.fromLTRB(16,14,16,8),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
      Text('Create with AI',style:Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight:FontWeight.w900)),const SizedBox(height:5),Text('Generate, preview and download images, videos and music.',style:TextStyle(color:cs.onSurfaceVariant)),const SizedBox(height:16),
