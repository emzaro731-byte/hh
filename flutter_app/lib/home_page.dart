...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> showIncomingCall(CallRecord call) async {
    ...
    try {
      ...
      callerName = '${profile?['display_name'] ?? 'GG User'}';
    } catch (_) {}
    if (!mounted) return;
  ...
  Future<String> createDirect(String other) async {
    ...
    for (final row in mine) {
      final members = await sb.from('conversation_members').select('user_id').eq('conversation_id', row['conversation_id']).eq('user_id', other).maybeSingle();
      if (members != null) {
              ...
              try {
                    ...
                    children: [
                      for (final item in users)
                        SimpleDialogOption(
                          ...
                          child: Text('${item['display_name'] ?? 'GG User'}'),
                        ),
                    ],
                  ),
                );
                if (selected != null && context.mounted) {
                  ...
                  await Navigator.push(context, MaterialPageRoute(builder: (_) => ChatPage(
                    conversation: Conversation(id: id, name: '${selected['display_name'] ?? 'GG User'}', message: '', time: ''),
                    dark: dark,
                  )));
                  refresh();
                }
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('GG', style: TextStyle(fontWeight: FontWeight.w900)),
            Text('Messenger', style: TextStyle(fontSize: 12)),
          ]),
          actions: [
