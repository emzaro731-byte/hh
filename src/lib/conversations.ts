...
export async function createGroupConversation(title:string,memberIds:string[]){const {data:{user},error:u}=await supabase.auth.getUser();if(u)throw u;if(!user)throw new Error('You must be signed in');const members=Array.from(new Set([user.id,...memberIds]));if(members.length<2)throw new Error('Select at least one member');const name=title.trim()||'GG Group';const {data:c,error}=await supabase.from('conversations').insert({is_group:true,title:name}).select('id').single();if(error)throw error;const {error:m}=await supabase.from('conversation_members').insert(members.map(userId=>({conversation_id:c.id,user_id:userId})));if(m)throw m;return c.id as string;}

export async function createDirectConversation(otherUserId:string){if(otherUserId.startsWith('__gg_group__:')){const ids=otherUserId.slice('__gg_group__:'.length).split(',').filter(Boolean);return createGroupConversation('GG Group',ids);}const {data:{user},error:u}=await supabase.auth.getUser();if(u)throw u;if(!user)throw new Error('You must be signed in');if(user.id===otherUserId)throw new Error('You cannot message yourself');const {data:mine}=await supabase.from('conversation_members').select('conversation_id').eq('user_id',user.id);for(const row of mine??[]){const {data:other}=await supabase.from('conversation_members').select('user_id').eq('conversation_id',row.conversation_id).eq('user_id',otherUserId).maybeSingle();if(other){const {data:c}=await supabase.from('conversations').select('id').eq('id',row.conversation_id).eq('is_group',false).maybeSingle();if(c)return c.id as string;}}const {data:c,error:cError}=await supabase.from('conversations').insert({is_group:false}).select('id').single();if(cError)throw cError;const {error:mError}=await supabase.from('conversation_members').insert([{conversation_id:c.id,user_id:user.id},{conversation_id:c.id,user_id:otherUserId}]);if(mError)throw mError;return c.id as string;}
